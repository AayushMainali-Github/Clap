...
let dVarUtcTime = (code, i) => {
  let returnData = {};
  let error = () => {
    returnData.data = `\nAn error occured while running the function dVarUtcTime at position ${i + 1}`;
    ...
    for (let j = i + 12; j < code.length; j++) {
      if (code[j] == ")" && code[j + 1] == ";") {
        ...
        try {
          ...
          variables.changeVariable(varName, `${utcDate.getUTCHours()}:${utcDate.getUTCMinutes()}:${utcDate.getUTCSeconds()}`);
          returnData.data = "";
