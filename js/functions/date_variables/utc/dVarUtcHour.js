...
let dVarUtcHour = (code, i) => {
  let returnData = {};
  let error = () => {
    returnData.data = `\nAn error occured while running the function dVarUtcHour at position ${i + 1}`;
  ...
  if (code[i + 11] == "(" && validLetters.includes(code[i + 12])) {
    for (let j = i + 12; j < code.length; j++) {
      if (code[j] == ")" && code[j + 1] == ";") {
        ...
        try {
          variables.changeVariable(varName, new Date().getUTCHours());
          returnData.data = "";
