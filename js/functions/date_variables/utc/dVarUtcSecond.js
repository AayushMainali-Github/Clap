...
let dVarUtcSecond = (code, i) => {
  let returnData = {};
  let error = () => {
    returnData.data = `\nAn error occured while running the function dVarUtcSecond at position ${i + 1}`;
  ...
  if (code[i + 13] == "(" && validLetters.includes(code[i + 14])) {
    for (let j = i + 14; j < code.length; j++) {
      if (code[j] == ")" && code[j + 1] == ";") {
        ...
        try {
          variables.changeVariable(varName, new Date().getUTCSeconds());
          returnData.data = "";
