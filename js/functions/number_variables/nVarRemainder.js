...
let nVarRemainder = (code, i) => {
  let returnData = {};
  let error = () => {
    returnData.data = `\nAn error occured while running the function nVarRemainder at position ${i + 1}`;
    returnData.jumpTo = code.length + 1;
  };
  if (code[i + 13] == "(" && validLetters.includes(code[i + 14])) {
    for (let j = i + 14; j < code.length; j++) {
      if (code[j] == ",") {
        let var1name = code.slice(i + 14, j);
