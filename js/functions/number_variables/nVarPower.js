...
let nVarPower = (code, i) => {
  let returnData = {};
  let error = () => {
    returnData.data = `\nAn error occured while running the function nVarPower at position ${i + 1}`;
      ...
      if (code[j] == ",") {
        ...
        for (let k = j + 1; k < code.length; k++) {
          if (code[k] == ")" && code[k + 1] == ";") {
            ...
            try {
              let nVarValue = math.evaluate(`${Math.pow(var1value.data, var2value.data)}`);
