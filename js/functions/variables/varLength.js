...
let varLength = (code, i) => {
  let returnData = {};
  let error = () => {
    returnData.data = `\nAn error occured while running the function varLength at position ${i + 1}`;
  ...
  if (code[i + 9] == "(" && validLetters.includes(code[i + 10])) {
    for (let j = i + 10; j < code.length; j++) {
      if (code[j] == ")" && code[j + 1] == ";") {
        ...
        try {
          variables.changeVariable(varName, varValue.data.toString().length);
          returnData.data = "";
